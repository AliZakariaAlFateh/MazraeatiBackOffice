...
namespace MazraeatiBackOffice.Controllers.FarmControllers
{
    public class FarmerReservationController : BaseController
    {
        ...
        //        {
        //            //هنا كمان المفروض يكون فيه فاليداشن كويس و هو انه لو كان فيه حجز فتره كاملة مسبقا فى نفس اليوم
        //            //لا يقبل حجز اطلاقا و كذلك اذا كان هناك اي حجز فى نفس اليوم صباحي او مسائي لا يقبل حجز يوم كامل تمام يا ريس 
        ...
        //            _UnitOfWork.FarmerReservationRepository.Insert(entity);
        //            _UnitOfWork.Save();
        ...
        //            //{
        //            //    { "type", "FarmReservation_notification" }
        //            //};
        ...
        //                Farm.UserId = 0;
        //            }
        //            await SendNotification((int)Farm.UserId, model.CustomerId, Farm.Name, model.CustomerName, 1);
        ...
        //        {
        //            logFile.LogCustomInfo("Create FarmerReservation - Inner Exception ", e.InnerException.ToString());
        //        }
        //        model.LookupValues = _LookupValueRepository.Table.Where(l => l.LookupId == 6).ToList();
        //        model.Customers = _UnitOfWork.CustomerRepository.Table.ToList();
        //        model.Farms = _FarmerRepository.Table.ToList();
        ...
        public async Task<IActionResult> Create(FarmerReservationModel model, IFormFile formFile)
        {
            ...
            try
            {
                ...
                {
                    // -------------------------------------------------------------
                    // 1. تطبيق منطق فحص التوفر الشامل (FullDay / Morning / Evening)
                    // -------------------------------------------------------------
                    // أنواع الحجز: 40 = FullDay, 41 = Morning, 42 = Evening
                    // حالات الحجز: 3 = Cancled (يتم استبعادها)

                    bool isAlreadyReserved = _FarmerReservationRepository.Table.Any(f =>
                        f.FarmerId == model.FarmerId &&
                        f.ReservationDate.Date == model.ReservationDate.Date &&
                        (int)f.ReservStatus != 3 && // تجاهل الحجوزات الملغية
                        (
                            f.ReservationTypeId == model.ReservationTypeId || // نفس الفترة تماماً
                            f.ReservationTypeId == 40 ||                      // يوجد حجز يوم كامل سابقاً
                            model.ReservationTypeId == 40                     // الحجز الحالي المطلوب هو يوم كامل
                        )
                    );

                    if (isAlreadyReserved)
                    {
                        ErrorNotification("عفواً، المزرعة محجوزة بالفعل في هذا اليوم/الفترة المختارة");
                        model.LookupValues = _LookupValueRepository.Table.Where(l => l.LookupId == 6).ToList();
                        model.Customers = _UnitOfWork.CustomerRepository.Table.ToList();
                        model.Farms = _FarmerRepository.Table.ToList();

                        return View(model); // استخدام View بدلاً من Redirect لعدم فقدان الـ ModelState والبيانات المدخلة
                    }

                    // -------------------------------------------------------------
                    // 2. جلب بيانات المزرعة والمالك مع حماية ضد الـ Null
                    // -------------------------------------------------------------
                    var farm = _FarmerRepository.Table.FirstOrDefault(f => f.Id == model.FarmerId);
                    if (farm == null)
                    {
                        ErrorNotification("المزرعة المختارة غير موجودة بالنظام");
                    ...
                    entity.ResponseDate = DateTime.Now;
                    entity.AutomaticallyNote = "تم الحجز من قبل محجوز بلوحة التحكم";
                    entity.IsMahjouzReservation = false; // افتراضي من الدواشبرد
                    ...
                    // 3. إرسال الإشعار
                    // -------------------------------------------------------------
                    int ownerUserId = farm.UserId ?? 0;
                    await SendNotification(ownerUserId, model.CustomerId, farm.Name, model.CustomerName, 0);
            ...
            {
                ErrorNotification($"حدث خطأ أثناء حفظ الحجز: {e.Message}");
                logFile.LogCustomInfo("Create FarmerReservation - Exception Message ", e.Message);
                logFile.LogCustomInfo("Create FarmerReservation - Stack Trace Message ", e.StackTrace);
        ...
        //            //المفروض كمان نرسل اشعار للعميل نفسه تمام الذي تم الحجز له ..
        ...
        //        if (e.InnerException != null)
        //        {
        //            logFile.LogCustomInfo("Edit Farmer Reservation - Inner Exception ", e.InnerException.ToString());
        //        }
        ...
        public async Task<IActionResult> Edit(FarmerReservationModel model, IFormFile formFile)
        {
            ...
            try
            {
                ...
                {
                    // -------------------------------------------------------------
                    // 1. تطبيق منطق فحص التوفر الشامل مع استثناء الحجز الحالي (Edit)
                    // -------------------------------------------------------------
                    // أنواع الحجز: 40 = FullDay, 41 = Morning, 42 = Evening
                    // حالات الحجز: 3 = Cancled (يتم استبعادها)

                    bool isAlreadyReserved = _FarmerReservationRepository.Table.Any(f =>
                        f.Id != model.Id &&                               // استثناء الحجز الحالي المراد تعديله
                        f.FarmerId == model.FarmerId &&
                        f.ReservationDate.Date == model.ReservationDate.Date &&
                        (int)f.ReservStatus != 3 &&                           // تجاهل الحجوزات الملغية
                        (
                            f.ReservationTypeId == model.ReservationTypeId || // نفس الفترة تماماً
                            f.ReservationTypeId == 40 ||                      // يوجد حجز يوم كامل آخر بنفس اليوم
                            model.ReservationTypeId == 40                     // التعديل الجديد المطلوب هو يوم كامل
                        )
                    );

                    if (isAlreadyReserved)
                    {
                        ErrorNotification("عفواً، توجد حجز آخر يتعارض مع هذا اليوم/الفترة المختارة");
                        model.LookupValues = _LookupValueRepository.Table.Where(l => l.LookupId == 6).ToList();
                        model.Customers = _UnitOfWork.CustomerRepository.Table.ToList();
                        model.Farms = _FarmerRepository.Table.ToList();
                        return View(model);
                    ...
                    model.MobileOwnerAppUser = user?.MobilePhone;
                    model.ModifiedDate = DateTime.Now;
                    model.AutomaticallyNote = "تم تحديث الحجز من قبل لوحة التحكم";

                    var entity = model.ToEntity();
                    _UnitOfWork.FarmerReservationRepository.Update(entity);
                    _UnitOfWork.Save();

                    // -------------------------------------------------------------
                    // 3. إرسال الإشعار لمالك المزرعة وللعميل
                    // -------------------------------------------------------------
                    int ownerUserId = farm.UserId ?? 0;

                    // إرسال إشعار لمالك المزرعة
                    await SendNotification(ownerUserId, model.CustomerId, farm.Name, model.CustomerName, 1);
            ...
            {
                ErrorNotification($"حدث خطأ أثناء تحديث الحجز: {e.Message}");
                logFile.LogCustomInfo("Edit FarmerReservation - Exception Message ", e.Message);
                logFile.LogCustomInfo("Edit FarmerReservation - Stack Trace Message ", e.StackTrace);
                ...
                return View(model); // استخدام View بدلاً من RedirectToAction للحفاظ على بيانات النموذج والخطأ
            }
        ...
        public IActionResult GetConfirmFarmReservationData(int id)
        {
            try
            {
                ...
                return PartialView("Partials/_ConfirmReservationFarmPopup", viewModel);
            }
            catch (Exception ex)
            {
                                                    ...
                                                    bool isReceiveCommission)
        {
            using (var transaction = _UnitOfWork.BeginTransaction(System.Data.IsolationLevel.ReadCommitted))
            {
                try
                {
                    ...
                    var objFarmerReserve = _UnitOfWork.FarmerReservationRepository
                                         .Table.Where(F => F.Id == reservationId).FirstOrDefault();

                    string reservedName = _UnitOfWork.FarmerRepository
                                         .Table.Where(F => F.Id == objFarmerReserve.FarmerId).FirstOrDefault().Name;
                    ...
                    var farmer = await _UnitOfWork.FarmerRepository.Table
                                .FirstOrDefaultAsync(F => F.Id == reservation.FarmerId);

                    string farmerName = farmer?.Name ?? "";
                    string title = " تأكيد حجز الشاليهات";
                    string source = "لوحة التحكم";

                    // {0} تأخذ farmerName، و {1} تأخذ source
                    string body = string.Format("\u200Fتم تأكيد حجزك على مزرعة {0}   (عبر {1} )",
                        farmerName,
                        source
                    );
        ...
        public async Task<IActionResult> ChangeStatus(int id, int status, string reason)
        {
            ...
            try
            {
                ...
                // {0} تأخذ farmerName، و {1} تأخذ source
                string body = string.Format("\u200Fتم إلغاء حجزك على مزرعة {0}، وذلك بسبب: {2} (عبر {1})",
                    farmerName,         // {0}
                    source,             // {1}
        ...
        private async Task SendNotification(int userId, int customerId, string realtyName, string customerName, int mode)
        {
            ...
            if (mode == 0)
            {
                ...
                data = new Dictionary<string, string>
                    {
                        { "type", "Farm_notification" }
                    };
                body = string.Format("\u200Fتم حجز المزرعة {0} بواسطة {1} (عبر {2})",
                       realtyName,
                       customerName,
                       source);
            ...
            if (mode == 1)
            {
                title = "تحديث حجز المزارع";
                data = new Dictionary<string, string>
                    {
                        { "type", "Farm_notification" }
                    };
                body = string.Format("\u200Fتم  تحديث حجز المزرعة {0} بواسطة {1} (عبر {2})",
                    realtyName,
                    customerName,
                    source);
            }
