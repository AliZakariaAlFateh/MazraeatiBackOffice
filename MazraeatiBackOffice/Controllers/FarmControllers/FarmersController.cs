...
namespace MazraeatiBackOffice.Controllers.FarmControllers
{
    ...
    public class FarmersController : BaseController
    {
        ...
        public FarmersController(IRepository<City> cityRepository, IRepository<Country> countryRepository, IRepository<Farmer> FarmerRepository,
            ...
            IRepository<Regions> regionRepository, IRepository<AppUser> userRepository,
        ...
        public FarmerModel EditFillModel(FarmerModel model)
        {
            ...
            model.Cities = _cityRepository.Table.Where(a => a.Active == true && a.CountryId == 2 && a.Id != 20).ToList();
            model.Users = _userRepository.Table.ToList();
            model.Regions = _regionRepository.Table.Where(r => r.Id == model.CityId).ToList();
        ...
        //        .Where(f => f.CountryId == 2 && !farmerBlackListIds.Contains(f.Id));
        ...
        //        // جرب تحويل النص لرقم
        //        bool isNumeric = int.TryParse(search, out int searchNumber) ||
        //                         int.TryParse(digitsOnly, out searchNumber);

        //        if (isNumeric && !string.IsNullOrEmpty(digitsOnly))
        //        {
        //            // ✅ أولوية: البحث برقم المزرعة بالضبط
        //            var farmByNumber = query.Where(a => a.Number == searchNumber);

        //            // ✅ ثانياً: البحث برقم الجوال (يبدأ بـ أو ينتهي بـ)
        //            var farmByMobile = query.Where(a =>
        //                a.MobileNumber.Contains(search) ||
        //                a.MobileNumber.StartsWith(digitsOnly) ||
        //                a.MobileNumber.EndsWith(digitsOnly)
        //            );

        //            // ✅ ثالثاً: البحث بالرقم التسلسلي (يبدأ بـ)
        //            //var farmBySerial = query.Where(a =>
        //            //    a.SerialFarmKey.StartsWith(digitsOnly)
        //            //);

        //            // دمج النتائج مع منع التكرار
        //            var allFarms = farmByNumber;//farmByNumber.Union(farmByMobile).Union(farmBySerial);
        //            query = allFarms;
        //        }
        //        else
        //        {
        //            // ✅ البحث بالنص (اسم أو غيره)
        //            query = query.Where(a =>
        ...
        //    // ✅ فلترة المدينة
        //    if (CityBy.HasValue && CityBy > 0)
        //    {
        //        query = query.Where(a => a.CityId == CityBy);
        //    }

        //    // ✅ فلترة التاريخ
        //    if (ReservationDate.HasValue)
        //    {
        //        var reservationIds = Reservation
        //            .Where(r => r.ReservationDate.Date == ReservationDate.Value.Date)
        //            .Select(r => r.FarmerId)
        //            .ToList();

        //        query = query.Where(a => !reservationIds.Contains(a.Id));
        //    }
        ...
        //PRG Pattern (Post → Redirect → Get) for search and index
        [HttpPost]
        public IActionResult Index(string search, int CityBy, DateTime ReservationDate)
        {
            ...
            {
                search = search,
                CityBy = CityBy == 0 ? (int?)null : CityBy,
        ...
        public IActionResult Farmer_IsApprove()
        {
            ...
            var farmerBlackListIds = _UnitOfWork.FarmerBlackListRepository.Table.Where(a => a.FarmerId != null && a.IsBlocked == true)
                                     .Select(a => a.FarmerId).ToList();
            var model = _FarmerRepository.Table.Where(f => f.CountryId == 2 &&  f.IsApprove == true && !farmerBlackListIds.Contains(f.Id)).OrderByDescending(a => a.Id)
                .Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback, Users, Regions));
           
            ViewBag.activePage = "المزارع الموافق عليها";
            ViewBag.cities = Cities.Where(c => c.CountryId == 2);
            ViewBag.DefaultDate = DateTime.Now;
            return View(model);
        }
        ...
        [HttpPost]
        public IActionResult Farmer_IsApprove(string search, int CityBy, DateTime ReservationDate)
        {
            if (string.IsNullOrEmpty(search) && CityBy == 0 && ReservationDate == DateTime.MinValue)
                return RedirectToAction("Farmer_IsApprove");
            ...
            if (ReservationDate != DateTime.MinValue)
            {
                ...
                model = _FarmerRepository.Table.OrderByDescending(a => a.Id).Where(a => (a.Name.Contains(search) ||
                                                                                        ...
                                                                                        (Reservation.Select(r => r.FarmerId).Contains(a.Id)) && a.CountryId == 2 &&  a.IsApprove == true
                                                                                   ).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback, Users, Regions));
            }
            else
            {
                model = _FarmerRepository.Table.OrderByDescending(a => a.Id).Where(a => (a.Name.Contains(search) ||
                                                                                                       a.MobileNumber.Contains(search) ||
                                                                                                       a.Number.ToString().Contains(search)) && (CityBy == 0 || (CityBy > 0 && a.CityId == CityBy))
                                                                                                       && a.CountryId == 2 && a.IsApprove == true
                                                                                   ).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback, Users, Regions));
            }
            ViewBag.activePage = "المزارع الموافق عليهم";
            ViewBag.search = search;
            ViewBag.CityBy = CityBy;
            ViewBag.cities = Cities.Where(c => c.CountryId == 2);
            ViewBag.DefaultDate = DateTime.Now;
        ...
        public IActionResult Farmer_Vip()
        {
            var Countries = _countryRepository.Table.Where(f => f.Id == 2).ToList();
            var Cities = _cityRepository.Table.Where(f => f.CountryId == 2).ToList();
            var Reservation = _FarmerReservation.Table.ToList();
            ...
            var farmerBlackListIds = _UnitOfWork.FarmerBlackListRepository.Table.Where(a => a.FarmerId != null && a.IsBlocked == true)
                                     .Select(a => a.FarmerId).ToList();
            var model = _FarmerRepository.Table.Where(f => f.CountryId == 2 &&  f.IsVIP == true && !farmerBlackListIds.Contains(f.Id)).OrderByDescending(a => a.Id)
                .Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback, Users, Regions));
          
            ViewBag.activePage = "المزارع الVIP";
            ViewBag.cities = Cities.Where(c => c.CountryId == 2);
            ViewBag.DefaultDate = DateTime.Now;
            return View(model);
        }
        ...
        [HttpPost]
        public IActionResult Farmer_Vip(string search, int CityBy, DateTime ReservationDate)
        {
            if (string.IsNullOrEmpty(search) && CityBy == 0 && ReservationDate == DateTime.MinValue)
                return RedirectToAction("Farmer_Vip");
            ...
            if (ReservationDate != DateTime.MinValue)
            {
                ...
                model = _FarmerRepository.Table.OrderByDescending(a => a.Id).Where(a => (a.Name.Contains(search) ||
                                                                                        a.MobileNumber.Contains(search) ||
                                                                                        a.Number.ToString().Contains(search)) && (CityBy == 0 || (CityBy > 0 && a.CityId == CityBy)) &&
                                                                                        (Reservation.Select(r => r.FarmerId).Contains(a.Id)) && a.CountryId == 2 && a.IsVIP == true
                                                                                   ).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback, Users, Regions));
            }
            else
            {
                model = _FarmerRepository.Table.OrderByDescending(a => a.Id).Where(a => (a.Name.Contains(search) ||
                                                                                                       a.MobileNumber.Contains(search) ||
                                                                                                       a.Number.ToString().Contains(search)) && (CityBy == 0 || (CityBy > 0 && a.CityId == CityBy))
                                                                                                       && a.CountryId == 2 && a.IsVIP == true
                                                                                   ).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback, Users, Regions));
            }
            ViewBag.activePage = "المزارع ال Vip";
            ViewBag.search = search;
            ViewBag.cities = Cities.Where(c => c.CountryId == 2);
            ViewBag.DefaultDate = DateTime.Now;
            return View(model);
        ...
        public IActionResult ReservationFarmers(int id, int? year = null, int? month = null)
        {
            ...
            IQueryable<FarmerReservation> filteredQuery;

            if (id == 1) // الشهر الحالي
            {
                filteredQuery = baseQuery.Where(r =>
                    r.ReservationDate.Date >= firstThis.Date &&
                    r.ReservationDate.Date < firstNext.Date);
            }
            else if (id == -1) // الشهر السابق
            {
                filteredQuery = baseQuery.Where(r =>
                    r.ReservationDate.Date >= firstPrev.Date &&
                    r.ReservationDate.Date < firstThis.Date);
            }
            else // الكل
            {
        ...
        public IActionResult FarmerPending()
        {
            ...
            var model = _FarmerRepository.Table.Where(f => f.CountryId == 2 && f.IsApprove == false).OrderByDescending(a => a.Id).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback,Users, Regions));
        ...
        public IActionResult FarmerPending(string search, int CityBy, DateTime ReservationDate)
        {
            ...
            if (ReservationDate != DateTime.MinValue)
            {
                ...
                model = _FarmerRepository.Table.OrderByDescending(a => a.Id).Where(a => (a.Name.Contains(search) ||
                                                                                        a.MobileNumber.Contains(search) ||
                                                                                        a.Number.ToString().Contains(search)) && (CityBy == 0 || (CityBy > 0 && a.CityId == CityBy)) &&
                                                                                        (Reservation.Select(r => r.FarmerId).Contains(a.Id)) && a.CountryId == 2 && a.IsApprove == false
                                                                                   ).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback,Users, Regions));
            }
            else
            {
                model = _FarmerRepository.Table.OrderByDescending(a => a.Id).Where(a => (a.Name.Contains(search) ||
                                                                                                       a.MobileNumber.Contains(search) ||
                                                                                                       a.Number.ToString().Contains(search)) && (CityBy == 0 || (CityBy > 0 && a.CityId == CityBy))
                                                                                                       && a.CountryId == 2 && a.IsApprove == false
                                                                                   ).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback,Users, Regions));
            }
            ViewBag.activePage = "المزارع الغير موافق عليهم";
            ViewBag.search = search;
        ...
        public IActionResult FarmerNotCompletedInfo()
        {
            ...
            var model = _FarmerRepository.Table.Where(f => f.CountryId == 2 && !FarmerImage.Contains(f.Id) || !FarmerPriceList.Contains(f.Id))
                                    .OrderByDescending(a => a.Id).Select(c => c.ToModel(Countries, Cities, Reservation, FarmerFeedback,Users, Regions));
        ...
        public IActionResult Create(FarmerModel model, IFormFile formFile)
        {
            ...
            try
            {
                ...
                if (model.CityId <= 0)
                {
                    ModelState.AddModelError("CityId", "برجاء اختيار  المدينة (المحافطة) من القائمة");
                }
                if (model.RegionId <= 0)
                {
                    ModelState.AddModelError("RegionId", "برجاء اختيار  المنظقة من القائمة");
                }
                //if (model.UserId <= 0)
                //{
                //    ModelState.AddModelError("UserId", "برجاء اختيار مستخدم  من القائمة");
                //}
                if (ModelState.IsValid)
                {
                    ...
                    if (model.Videos != null)
                    {
                        foreach (IFormFile file in model.Videos)
                        {
                            ...
                            if (file != null)
                                farmerVideo.Url = "jordanfarms/" + GenericFunction.UploadedVideo(file, webHostEnvironment, "jordanfarms");
            ...
            catch (Exception e)
            {
                ...
                //var errorMessage = $"Error while saving farms: {e.Message}. Please contact the administrator.";
        ...
        public IActionResult Edit(int id)
        {
            ...
            ViewBag.RegionId = farmer.RegionId;
            ViewBag.activePage = "المزارع";
        ...
        public IActionResult Edit(FarmerModel model, IFormFile formFile)
        {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (model.Videos != null)
                    {
                        foreach (IFormFile file in model.Videos)
                        {
                            ...
                            if (file != null)
                                farmerVideo.Url = "jordanfarms/" + GenericFunction.UploadedVideo(file, webHostEnvironment, "jordanfarms");
                                    //GenericFunction.UploadedVideo(file, webHostEnvironment);
                    ...
                    //int status = (int)model.statusFarmAppUser;
                    //model.statusFarmAppUser = (FarmAppUserStatus)status;
                    _UnitOfWork.FarmerRepository.Update(model.ToEntity());
                    _UnitOfWork.Save();
            ...
            catch (Exception e)
            {
                ...
                ErrorNotification($"Error while Update farms: {e.Message}. Please contact the administrator.");
                //var errorMessage = e.InnerException != null
                //                    ? e.InnerException.Message
                //                    : e.Message;
                //var errorMessage = $"Error while update farms: {e.Message}. Please contact the administrator.";
                //TempData["ErrorMessage"] = errorMessage;
                logFile.LogCustomInfo("Edit Farmer - Exception Message ", e.Message);
                logFile.LogCustomInfo("Edit Farmer - Stack Trace Message ", e.StackTrace);
                logFile.LogCustomInfo("Edit Farmer - Inner Exception Message ", e.InnerException.ToString());
        ...
        public IActionResult DeleteFarmerImage(int imageId)
        {
            try
            {
                ...
                string imagePath = Path.Combine(webHostEnvironment.WebRootPath, "Images", imageToDelete.Url.Replace("/", "\\").TrimStart('\\'));
        ...
        public IActionResult DeleteFarmerVideo(int videoId)
        {
            try
            {
                ...
                string videoPath = Path.Combine(webHostEnvironment.WebRootPath,"Videos", videoToDelete.Url.Replace("/", "\\").TrimStart('\\'));
        ...
        {
            var regions = _regionRepository.Table
                .Where(r => r.CityId == cityId)
                .Select(r => new { id = r.Id, descAr = r.DescAr , descEn =r.DescEn })
                .ToList();
