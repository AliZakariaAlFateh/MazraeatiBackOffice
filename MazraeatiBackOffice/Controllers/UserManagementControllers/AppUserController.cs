...
namespace MazraeatiBackOffice.Controllers.UserManagementControllers
{
    ...
    public class AppUserController : BaseController
    {
        ...
        public UserModel FillModel(UserModel model)
        {
            ...
            // ===== جلب الأكواخ =====
            var cottages = _UnitOfWork.CottageRepository.Table.ToList();
            model.Cottages = cottages.Select(x => new SelectListItem
            {
                Value = x.Id.ToString(),
                Text = x.NameAr
            }).ToList();
            ...
            if (model.Id > 0)
            {
                ...
                var selectedCottages = _UnitOfWork.CottageRepository.Table
                    .Where(f => f.UserId == model.Id)
                    .Select(f => f.Id)
                    .ToList();
                    model.CottageIds = selectedCottages;
            }
        ...
        public IActionResult Index(string search, int? userType, bool? isActive)
        {
            ...
            var userTypesList = new List<SelectListItem>
            {
                ...
                new SelectListItem { Value = "11", Text = "مالك اسكواش" },
                new SelectListItem { Value = "12", Text = "مالك ريشة طائرة" },
                new SelectListItem { Value = "13", Text = "مالك  كوخ" }
            };
        ...
        public IActionResult Create([FromForm] UserModel model, IFormFile formFile)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    // ===== ربط الأكواخ =====
                    if (model.CottageIds != null && model.CottageIds.Any())
                    {
                        var cottages = _UnitOfWork.CottageRepository.Table
                            .Where(x => model.CottageIds.Contains(x.Id))
                            .ToList();
                        foreach (var cottage in cottages)
                        {
                            cottage.UserId = userId;
                            cottage.MobileOwnerAppUser = mobileOwner;
        ...
        //    model.Cottages = _UnitOfWork.CottageRepository.Table
        //        .Select(x => new SelectListItem
        //        {
        //            Value = x.Id.ToString(),
        //            Text = x.NameAr
        ...
        public IActionResult Edit(int id)
        {
            ...
            // ===== استدعاء FillModel بعد ما ضبطنا كل حاجة =====
            model = FillModel(model); // ✅ دلوقتي هتضبط UserTypeListSelected وغيرها
        ...
        public IActionResult Edit(UserModel model, IFormFile formFile)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    // ===== تحديث الأكواخ =====
                    var oldCottage = _UnitOfWork.CottageRepository.Table
                        .Where(x => x.UserId == model.Id)
                        .ToList();
                    var newCottageIds = model.CottageIds ?? new List<int>();

                    var removedCottage = oldCottage.Where(x => !newCottageIds.Contains(x.Id)).ToList();
                    foreach (var cottage in removedCottage)
                    {
                        cottage.UserId = null;
                    ...
                    var cottagesToAdd = _UnitOfWork.CottageRepository.Table
                        .Where(x => newCottageIds.Contains(x.Id) && x.UserId != model.Id)
                        .ToList();
                    foreach (var cottage in cottagesToAdd)
                    {
                        cottage.UserId = model.Id;
                        cottage.MobileOwnerAppUser = MobileOwner;
                        _UnitOfWork.CottageRepository.Update(cottage);
                    }
            ...
            return new List<SelectListItem>
    {
        ...
        // ============================================================
        // جلب الأكواخ حسب النوع (AJAX)
        // ============================================================
        public IActionResult GetCottagesByUserType(string userType)
        {
            if (string.IsNullOrEmpty(userType) || userType != "13")
                return Json(new List<object>());
