...
namespace MazraeatiBackOffice.Controllers
{
    public class CityController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            var model = _cityRepository.Table.Where(c => c.CountryId == 2).OrderByDescending(a => a.Id).Select(c => c.ToModel(Countries));
        ...
        public IActionResult Index(string search)
        {
            ...
            var Countries = _countryRepository.Table.ToList();
            var model = _cityRepository.Table.Where(c => c.CountryId == 2).OrderByDescending(a => a.Id).Where(a => a.DescAr.Contains(search) || a.DescEn.Contains(search)).Select(c => c.ToModel(Countries));
        ...
        public IActionResult Delete(int id)
        {
            ...
            var infarmerExists = _UnitOfWork.FarmerRepository.Table
                        .Where(x => x.CityId == id)
                        .ToList();
            var inRegionExists = _UnitOfWork.RegionRepository.Table
            .Where(x => x.CityId == id)
            .ToList();
            if (infarmerExists.Count == 0 && inRegionExists.Count == 0)
            {
                City city = _cityRepository.GetById(id);
                if (city == null)
            ...
            {
                return Json("لا يمكن حذف هذه المحافظة لانه مربوطة بمزرعة أو أنها توجد لها مناطق");
            }
