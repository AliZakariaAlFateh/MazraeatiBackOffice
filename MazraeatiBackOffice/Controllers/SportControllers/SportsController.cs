...
namespace MazraeatiBackOffice.Controllers.SportControllers
{
    public class SportsController : BaseController
    {
        ...
        public IActionResult Create(int? SportTypeId)
        {
            ViewBag.activePage = "الأنشطة الرياضية";
            var sportTypes = _UnitOfWork.SportTypeRepository.Table.Where(s => s.IsActive == true).ToList();
            ViewBag.SportTypeTitle = ((UserTypeEnumTitle)SportTypeId.Value).GetDisplayName();
        ...
        public IActionResult Create(SportModel model)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (model.Images != null)
                    {
                        foreach (IFormFile file in model.Images)
                        {
                            if (file != null && file.Length > 0)
                            {
                                ...
                                {
                                    SportId = nSportId,
                                    Url = $"sports/{nSportId}/" + GenericFunction.UploadedFile(file, _webHostEnvironment, $"sports/{nSportId}"),
                                    Sort = nSortImage,
                                    Vip = true,
                                    Active = true
                                });
                    ...
                    if (model.Videos != null)
                    {
                        foreach (IFormFile file in model.Videos)
                        {
                            if (file != null && file.Length > 0)
                            {
                                ...
                                {
                                    SportId = nSportId,
                                    Url = $"sports/{nSportId}/" + GenericFunction.UploadedVideo(file, _webHostEnvironment, $"sports/{nSportId}"),
                                    Sort = nSortVideo,
                                    Active = true
                                });
                    ...
                    return RedirectToAction("Index", new { model.SportTypeId });
                }
            }
            catch (Exception e)
            {
                ...
                ViewBag.SportTypeTitle = ((UserTypeEnumTitle)model.SportTypeId).GetDisplayName();
                //return RedirectToAction("Create", new { id = model.Id });
                //new { sportTypeId = model.SportTypeId }
                return RedirectToAction("Create", new { model.SportTypeId });
            }
        ...
        public IActionResult Edit(int id)
        {
            ...
            ViewBag.SportTypeId = sport.SportTypeId;
            ViewBag.SelectedSportTypeId = sport.SportTypeId;
            ViewBag.activePage = "الأنشطة الرياضية";
            ViewBag.SportTypeTitle = ((UserTypeEnumTitle)sport.SportTypeId).GetDisplayName();
            // ===== إضافة هذا السطر =====
            ViewData["SelectedSportTypeId"] = sport.SportTypeId;
        ...
        public IActionResult Edit(SportModel model)
        {
                    ...
                    if (model.Images != null)
                    {
                        foreach (IFormFile file in model.Images)
                        {
                            if (file != null && file.Length > 0)
                            {
                                ...
                                {
                                    SportId = model.Id,
                                    Url = $"sports/{model.Id}/" + GenericFunction.UploadedFile(file, _webHostEnvironment, $"sports/{model.Id}"),
                                    Sort = nSortImage,
                                    Vip = true,
                                    Active = true
                                });
                    ...
                    if (model.Videos != null)
                    {
                        foreach (IFormFile file in model.Videos)
                        {
                            if (file != null && file.Length > 0)
                            {
                                ...
                                {
                                    SportId = model.Id,
                                    Url = $"sports/{model.Id}/" + GenericFunction.UploadedVideo(file, _webHostEnvironment, $"sports/{model.Id}"),
                                    Sort = nSortVideo,
                                    Active = true
                                });
                                _UnitOfWork.Save();
            ...
            ViewBag.SportTypeTitle = ((UserTypeEnumTitle)model.SportTypeId).GetDisplayName();
            ViewBag.RegionId = model.RegionId;
            ViewBag.SportTypeId = model.SportTypeId;
            ViewData["SelectedSportTypeId"] = model.SportTypeId;
            model = EditFillModel(model);
            return View(model);
        }
        ...
        public SportModel NewFillModel(SportModel model, int? SportTypeId = null)
        {
            ...
            model.AdditionalServices.Clear();
        ...
        public IActionResult SportPriceList(int sportId,int? SportTypeId)
        {
            if (sportId == 0)
            {
                ...
                ErrorNotification("الرجاء حفظ النشاط الرياضي أولاً");
                return RedirectToAction("Create", "Sports", new { SportTypeId });
            }
