...
namespace MazraeatiBackOffice.Controllers.SystemControllers
{
    public class RegionController : BaseController
    {
        ...
        public RegionController(IUnitOfWork unitOfWork,IRepository<Regions> regionRepository,
            IRepository<City> cityRepository, IWebHostEnvironment hostEnvironment)
        {
        ...
        public IActionResult Index(int? cityid)
        {
            ...
            ViewBag.CreateUrl = Url.Action("Create", "Region", new { cityid });
            return View(model);
        }
        ...
        {
            Regions region = _regionRepository.GetById(id);
            if (region == null)
                return RedirectToAction("Index");
        ...
        {
            // search about any region assign in a farm
            var result= _UnitOfWork.FarmerRepository.Table
                    .Where(x => x.RegionId == id)
                    .ToList();
            if (result.Count == 0 )
            {
                //SuccessNotification(result.First().Name);
                Regions region = _regionRepository.GetById(id);
                if (region == null)
            ...
            {
                return Json("لا يمكن حذف هذه المنطقة لانها مربوطة بمزرعة");
            }
